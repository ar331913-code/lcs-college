...
localData.aboutIntroImage = './images/aboutIntroImage.jpg';
localData.updatedAt = Date.now();
fs.writeFileSync(siteDataPath, JSON.stringify(localData, null, 2), 'utf8');
console.log('Updated src/siteData.json with aboutIntroImage');
...
https.get(binUrl, (res) => {
  ...
  res.on('end', () => {
    try {
      ...
      parsed.aboutIntroImage = './images/aboutIntroImage.jpg';
